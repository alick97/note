...
function getTmpDir() {
    try {
        return fs_1.default.realpathSync.native(os_1.default.tmpdir());
    }
    catch {
...
function genTmpFile() {
    return (0, promise_1.default)((async () => {
        const tmpDir = getTmpDir();
        for (let i = 0; i < 10; i++) {
            const name = `honkit-${process.pid}-${Date.now()}-${crypto_1.default.randomBytes(6).toString("hex")}`;
            const filePath = path_1.default.join(tmpDir, name);
            let handle;
            try {
                handle = await fs_1.default.promises.open(filePath, fs_1.default.constants.O_CREAT | fs_1.default.constants.O_EXCL | fs_1.default.constants.O_RDWR, 0o600);
                return filePath;
            }
            catch (error) {
 ...
 */
function genTmpDir() {
    return (0, promise_1.default)(fs_1.default.promises.mkdtemp(path_1.default.join(getTmpDir(), "honkit-")));
}
